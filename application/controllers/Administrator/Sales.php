...
class Sales extends CI_Controller
{
        ...
        try {
            ...
            if (isset($data->customer)) {
                ...
                if ($type == 'N') {
                    ...
                    $customer['Customer_Credit_Limit'] = 500000;
                }

                $customer['Customer_Code'] = $this->mt->generateCustomerCode();
                $customer['status'] = 'a';
                $customer['owner_name'] = $data->customer->Customer_Name;
    ...
    public function getSales()
    {
        ...
        $sales = $this->db->query("
            select 
            concat(sm.SaleMaster_InvoiceNo, ' - ', c.Customer_Mobile) as invoice_text,
            sm.*,
            c.Customer_Code,
            c.Customer_Name,
            c.Customer_Mobile,
...
<?php
    ...
    public function dueReminder()
    {
        ...
        $data['content'] = $this->load->view('Administrator/due_report/due_reminder', $data, true);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getDueReminders()
    {
        ...
        $dues = $this->db->query("
            ...
            where sm.Status = 'a'
            and sm.SaleMaster_branchid = ?
            and sm.reminder_date between ? and ?
        ", [$this->session->userdata('BRANCHid'), $data->dateFrom, $data->dateTo])->result();

        echo json_encode($dues);
    }

    public function changeReminderDate()
    {
        $res = new stdClass;
        try {
            $data = json_decode($this->input->raw_input_stream);

            $this->db->where('SaleMaster_SlNo', $data->salesId)->set('reminder_date', $data->date)->update('tbl_salesmaster');

            $res->message = 'Reminder date update successfully';
        } catch (\Exception $e) {
